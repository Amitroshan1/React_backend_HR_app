...
class Admin(db.Model, UserMixin):
    ...
    # Login allowed until this date after exit (future last working day).
    ...
    payroll_records = db.relationship(
        "MonthlyPayroll",
        foreign_keys="MonthlyPayroll.admin_id",
        back_populates="admin",
        uselist=True,
        cascade="all, delete-orphan",
    )
...
class EmployeeArchive(db.Model):
    ...
    rehire_eligible = db.Column(db.Boolean, nullable=True, default=True)
    rehire_cooldown_until = db.Column(db.Date, nullable=True)
    rehire_notes = db.Column(db.Text, nullable=True)
...
class EmployeeExitHistory(db.Model):
    ...
    notice_shortfall_days = db.Column(db.Integer, nullable=True, default=0)
    resignation_date_snapshot = db.Column(db.Date, nullable=True)
    force_override = db.Column(db.Boolean, nullable=True, default=False)
    force_override_reason = db.Column(db.Text, nullable=True)
